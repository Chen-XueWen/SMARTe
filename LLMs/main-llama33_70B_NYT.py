...
# Define a function that adds a delay to a Completion API call to prevent token rate limit
def delayed_completion(model, messages, delay_in_seconds: float = 5):
...
def extract_triples_with_llm(text):
    ...
    """
    RELATIONS = [
        "industry", "place_lived", "founders", "advisors", "children",
        "neighborhood_of", "people", "profession", "place_of_birth",
        "ethnicity", "teams", "company", "administrative_divisions",
        "place_of_death", "geographic_distribution", "major_shareholders",
        "nationality", "place_founded", "capital", "country", "religion",
        "major_shareholder_of", "contains", "location"
    ]
...
"""
    response = delayed_completion(delay_in_seconds=5,
